...
export default function AdminContent({ db, employeeEmail, hasPermission, onRefresh }: AdminContentProps) {
  ...
  const handleDeleteNews = async (id: string) => {
    if (!canManage) return;
    if (confirm('Tem a certeza de que deseja apagar esta notícia permanentemente?')) {
  ...
  const handleDeleteShow = async (id: string) => {
    if (!canManage) return;
    if (confirm('Tem a certeza de que deseja remover este programa da grelha de transmissão?')) {
